...
| Object set under `excludeOnEdges` | A | 3 | signoff | - | **The direction was wrong until Stage 03 enumerated it.** It was declared from the Stage 2 bytecode reading — `Counter3D.findObjects` flags edge contact against a provisional id, `replaceID` does not carry the flag across a later merge, so an edge-touching object survives the filter — which predicts `Counter3D` keeping objects the new engine drops. `ExcludeOnEdgesEnumerationTest` runs all 33 Case A fixtures plus 1 500 seeded random volumes through both engines and finds **no such object**. What it does find runs the other way: `Counter3D` drops objects that touch no border at all, whenever the volume's final voxel in scan order is an isolated object. That is the `IDcount` off-by-one of the row below, which throws with this option off and silently loses an object with it on — reduced to **two voxels**, and isolated to scan-order finality rather than edge-ness by moving the corner one slice earlier, where the engines agree again. Still a fix, and still Tier 3: the new engine keeps an object that should never have been dropped. The merge-loses-the-flag path is not disproved, only unobserved here. With `excludeOnEdges` off — the default — Tier 1 stands unchanged. |
